...
# buradaki işlem listesi bloğa eklenmemiş işlemlerdir   
    def add_transaction(self, sender, receiver, message): # İşlemlerin kaydının tutulduğu bir işlev ex:gönderici,alıcı,miktar
        self.transactions.append({'sender': sender, # Gönderici
                                  'receiver': receiver, # Alıcı
                                  'message': message}) # Miktar,
...
def add_transaction():
    ...
    transaction_keys = ['sender', 'receiver', 'message'] #İşlem anahtarları kim yaptı,kime hangi işlem ,tutar
...
# Runnuing the app
app.run(host= '0.0.0.0', port = 5000)  # port numarası verilerek işlemlerin farklı düğümlerde de gözükmesi yani merkeziyetsizleştirimesi gerekir 
